...
public sealed class LocalJsonCharacterRepositoryTests
{
    ...
    [Fact]
    public async Task UnsupportedSchemaVersionIsLoadableButNeedsReview()
    {
        using var temp = new TempDirectory();
        var ruleset = await TestFixtures.GetRulesetAsync();
        var character = TestFixtures.CompleteCharacter(ruleset) with { SchemaVersion = Character.CurrentSchemaVersion + 1 };
        var repository = new LocalJsonCharacterRepository(temp.Path);
        ...
        Assert.Equal("character.unsupportedSchemaVersion", loaded.Error?.Code);
        var entry = Assert.Single(entries);
        Assert.True(entry.NeedsReview);
        Assert.False(entry.IsLoadable);
    }
